/* empty css                      */import{a as y,i as d}from"./assets/vendor-CT17xhyR.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))a(s);new MutationObserver(s=>{for(const c of s)if(c.type==="childList")for(const r of c.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function i(s){const c={};return s.integrity&&(c.integrity=s.integrity),s.referrerPolicy&&(c.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?c.credentials="include":s.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function a(s){if(s.ep)return;s.ep=!0;const c=i(s);fetch(s.href,c)}})();const $="https://66b1f8e71ca8ad33d4f5f63e.mockapi.io/campers";document.querySelector(".loader");async function E(){try{return(await y.get($)).data.items}catch(e){throw e}}async function v(e){try{return(await y.get(`${$}/${e}`)).data}catch(t){throw t}}function w(e){return`<li class="campers-item">
   ...
   <div class="form-container">
    ...
    `}async function F(e){const t=await v(e),i=S();return` <section class='section-reviews'>
          ...
          ${i}
        </section>`}async function x(e){document.body.classList.add("no-scroll");try{const t=await v(e);if(!t){d.show({message:"Camper not found",timeout:5e3,position:"topRight",color:"red"});return}const i=document.querySelector(".modal-content");if(!i){d.show({message:"Error loading modal content. Please try again later.",timeout:5e3,position:"topRight",color:"red"});return}i.innerHTML=await R(t),document.querySelector(".modal").classList.add("open"),N();const a=document.querySelector("#bookingForm");a.addEventListener("submit",function(s){s.preventDefault(),d.show({message:"Success! We have received your booking request. We will contact you shortly.",timeout:5e3,position:"topRight",color:"green"});const c=new FormData(a),r=c.get("name"),m=c.get("email"),p=c.get("bookingDate");console.log("Booking request:",{name:r,email:m,date:p,camperId:e}),a.reset()})}catch{d.show({message:"Error fetching camper data. Please try again later.",timeout:5e3,position:"topRight",color:"red"})}}function I(){document.body.classList.remove("no-scroll"),document.querySelector(".modal").classList.remove("open")}document.addEventListener("click",function(e){(e.target.classList.contains("close-modal")||e.target.classList.contains("modal"))&&I()});async function B(e,t){const i={features:await q(t),reviews:await F(t)},a=document.getElementById("outlet");a&&(a.innerHTML=i[e]||"<p>Page not found</p>"),document.querySelectorAll(".nav-link").forEach(c=>c.classList.remove("active"));const s=document.querySelector(`.nav-link[data-page="${e}"]`);s&&s.classList.add("active")}function N(){document.querySelectorAll(".nav-link").forEach(e=>{e.addEventListener("click",function(t){t.preventDefault();const i=t.target.dataset.id;B(t.target.dataset.page,i)})})}document.addEventListener("click",function(e){if(e.target.closest(".btn-like")){const t=e.target.closest(".btn-like"),i=t.dataset.id;t.classList.toggle("liked");const a=t.querySelector("img"),s=t.classList.contains("liked");a.src=s?"red-heart.svg":"heart.svg";const c=JSON.parse(localStorage.getItem("likedCampers"))||{};c[i]=s,localStorage.setItem("likedCampers",JSON.stringify(c))}});const O=document.querySelector(".result"),u=document.querySelector(".loader"),f=document.querySelector(".load-more"),D=document.querySelector("#filtersForm");u.style.display="none";f.style.display="none";let l=1;const n=4;let g=[],o=[];async function P(){try{u.style.display="block";const e=await E();u.style.display="none",g=e,o=g,L(o,n),l=1,b(l,n,o)}catch{O.innerHTML="<p>Not found</p>",f.classList.add("no-active")}}function H(){l++;const e=(l-1)*n,t=l*n;A(o,e,t),b(l,n,o)}D.addEventListener("submit",function(e){e.preventDefault(),o=M(g),L(o,n)});document.querySelectorAll(".container-filters").forEach(e=>{e.addEventListener("click",function(){e.classList.toggle("active-filter")})});document.addEventListener("click",function(e){const t=e.target.closest(".campers-link");if(!t)return;const i=t.dataset.id;x(i)});f.addEventListener("click",H);window.onload=P;
