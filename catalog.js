/* empty css                      */import{a as v}from"./assets/vendor-BcYoTxAJ.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))c(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&c(r)}).observe(document,{childList:!0,subtree:!0});function s(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function c(i){if(i.ep)return;i.ep=!0;const a=s(i);fetch(i.href,a)}})();const f="https://66b1f8e71ca8ad33d4f5f63e.mockapi.io/campers",d=document.querySelector(".loader");async function q(){try{d.style.display="block";const e=await v.get(f);return d.style.display="none",e.data.items}catch(e){throw console.error("Error:",e),e}}async function p(e){try{d.style.display="block";const t=await v.get(`${f}/${e}`);return d.style.display="none",t.data}catch(t){throw console.error("Error:",t),t}}function h(e){return`<li class="campers-item">
          ...
          </li>`}const y=document.querySelector(".result"),b=document.querySelector(".load-more");function $(e,t){y.innerHTML=e.slice(0,t).map(h).join("");const s=JSON.parse(localStorage.getItem("likedCampers"))||{};document.querySelectorAll(".btn-like").forEach(c=>{const i=c.dataset.id;s[i]&&(c.classList.add("liked"),c.querySelector("img").src="red-heart.svg")})}function E(e,t,s){y.insertAdjacentHTML("beforeend",e.slice(t,s).map(h).join(""))}function w(e,t,s){b.style.display=e*t>=s.length?"none":"block"}function T(e){var i,a;const t=((i=document.querySelector("#location"))==null?void 0:i.value.toLowerCase())||"",s=[...document.querySelectorAll(".container-filters.active-filter")].map(r=>r.id),c=((a=document.querySelector('input[name="vehicleType"]:checked'))==null?void 0:a.value)||"";return e.filter(r=>{const k=t?r.location.toLowerCase().includes(t):!0,S=s.every(g=>r[g]===!0||r[g]==="automatic"),C=c?r.form===c:!0;return k&&S&&C})}async function L(e){const t=await p(e);return`<section class='features'>
            ${t?`<div>
        ...
        </section>`}async function A(e){const t=await L(e.id);return`
    <section class='camp'>
      ...
      <div class='container-camp'>
          <ul class='gallery-camp'>
              ${e.gallery.map((s,c)=>`
                <li>
                  <img class='image-camp' src=${s.original} alt="image ${c+1}" />
                </li>
              `).join("")}
          </ul>
      </div>
