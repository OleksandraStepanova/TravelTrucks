/* empty css                      */import{a as f}from"./assets/vendor-BcYoTxAJ.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))r(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const c of i.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&r(c)}).observe(document,{childList:!0,subtree:!0});function s(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function r(a){if(a.ep)return;a.ep=!0;const i=s(a);fetch(a.href,i)}})();const v="https://66b1f8e71ca8ad33d4f5f63e.mockapi.io/campers",d=document.querySelector(".loader");async function C(){try{d.style.display="block";const e=await f.get(v);return d.style.display="none",e.data.items}catch(e){throw console.error("Error:",e),e}}async function q(e){try{d.style.display="block";const t=await f.get(`${v}/${e}`);return d.style.display="none",t.data}catch(t){throw console.error("Error:",t),t}}function h(e){return`<li class="campers-item">
    ...
    `}async function m(e){try{const t=await q(e);if(!t){console.error("Camper not found");return}const s=document.querySelector(".modal-content");if(!s){console.error("Modal content not found in DOM");return}s.innerHTML=I(t),document.querySelector(".modal").classList.add("open"),sessionStorage.setItem("openModal",e),window.history.pushState({},"",`/catalog.html/${e}`),O()}catch(t){console.error("Error fetching camper data:",t)}}function T(){document.querySelector(".modal").classList.remove("open"),window.history.pushState({},"","/catalog.html"),sessionStorage.removeItem("openModal")}document.addEventListener("click",function(e){(e.target.classList.contains("close-modal")||e.target.classList.contains("modal"))&&T()});function A(e,t){const s={features:`<h1>Features Page</h1><p>Features of camper ID: ${t}</p>`,reviews:`<h1>Reviews Page</h1><p>Reviews for camper ID: ${t}</p>`},r=document.getElementById("outlet");r&&(r.innerHTML=s[e]||"<h1>Catalog</h1>"),document.querySelectorAll(".nav-link").forEach(i=>i.classList.remove("active"));const a=document.querySelector(`.nav-link[data-page="${e}"]`);a&&a.classList.add("active")}function O(){document.querySelectorAll(".nav-link").forEach(e=>{e.addEventListener("click",function(t){t.preventDefault();const s=t.target.dataset.id;A(t.target.dataset.page,s)})})}document.addEventListener("click",function(e){if(e.target.closest(".btn-like")){const t=e.target.closest(".btn-like"),s=t.dataset.id;t.classList.toggle("liked");const r=t.querySelector("img"),a=t.classList.contains("liked");r.src=a?"./src/img/red-heart.svg":"./src/img/heart.svg";const i=JSON.parse(localStorage.getItem("likedCampers"))||{};i[s]=a,localStorage.setItem("likedCampers",JSON.stringify(i))}});const P=document.querySelector(".result"),F=document.querySelector(".loader"),p=document.querySelector(".load-more"),R=document.querySelector("#filtersForm");F.style.display="none";p.style.display="none";let l=1;const n=4;let u=[],o=[];async function N(){try{u=await C(),o=u,$(o,n),l=1,w(l,n,o)}catch{P.innerHTML="<p>Not found</p>",p.classList.add("no-active")}}function B(){l++;const e=(l-1)*n,t=l*n;M(o,e,t),w(l,n,o)}R.addEventListener("submit",function(e){e.preventDefault(),o=b(u),$(o,n)});document.querySelectorAll(".container-filters").forEach(e=>{e.addEventListener("click",function(){e.classList.toggle("active-filter")})});document.addEventListener("click",function(e){const t=e.target.closest(".campers-link");if(!t)return;const s=t.dataset.id;window.history.pushState({},"",`/catalog.html/${s}`),m(s)});document.addEventListener("DOMContentLoaded",()=>{const e=window.location.pathname.split("/").filter(Boolean),s=e[0]==="catalog.html"&&e[1]?e[1]:null,r=sessionStorage.getItem("openModal");s?m(s):r&&m(r)});p.addEventListener("click",B);window.onload=N;
