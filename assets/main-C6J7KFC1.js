import{a as f}from"./vendor-BcYoTxAJ.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))c(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&c(r)}).observe(document,{childList:!0,subtree:!0});function s(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function c(a){if(a.ep)return;a.ep=!0;const i=s(a);fetch(a.href,i)}})();const v="https://66b1f8e71ca8ad33d4f5f63e.mockapi.io/campers",d=document.querySelector(".loader");async function C(){try{d.style.display="block";const e=await f.get(v);return d.style.display="none",e.data.items}catch(e){throw console.error("Error:",e),e}}async function q(e){try{d.style.display="block";const t=await f.get(`${v}/${e}`);return d.style.display="none",t.data}catch(t){throw console.error("Error:",t),t}}function h(e){return`<li class="campers-item">
          ...
          </li>`}const y=document.querySelector(".result"),E=document.querySelector(".load-more");function $(e,t){y.innerHTML=e.slice(0,t).map(h).join("");const s=JSON.parse(localStorage.getItem("likedCampers"))||{};document.querySelectorAll(".btn-like").forEach(c=>{const a=c.dataset.id;s[a]&&(c.classList.add("liked"),c.querySelector("img").src="./src/img/red-heart.svg")})}function M(e,t,s){y.insertAdjacentHTML("beforeend",e.slice(t,s).map(h).join(""))}function w(e,t,s){E.style.display=e*t>=s.length?"none":"block"}function b(e){var a,i;const t=((a=document.querySelector("#location"))==null?void 0:a.value.toLowerCase())||"",s=[...document.querySelectorAll(".container-filters.active-filter")].map(r=>r.id),c=((i=document.querySelector('input[name="vehicleType"]:checked'))==null?void 0:i.value)||"";return e.filter(r=>{const L=t?r.location.toLowerCase().includes(t):!0,S=s.every(g=>r[g]===!0||r[g]==="automatic"),k=c?r.form===c:!0;return L&&S&&k})}function I(e){return`
    <section class='camp'>
    ...
    `}async function m(e){try{const t=await q(e);if(!t){console.error("Camper not found");return}const s=document.querySelector(".modal-content");if(!s){console.error("Modal content not found in DOM");return}s.innerHTML=I(t),document.querySelector(".modal").classList.add("open"),sessionStorage.setItem("openModal",e),window.history.pushState({},"",`/catalog/${e}`),O()}catch(t){console.error("Error fetching camper data:",t)}}function T(){document.querySelector(".modal").classList.remove("open"),window.history.pushState({},"","/catalog"),sessionStorage.removeItem("openModal")}document.addEventListener("click",function(e){(e.target.classList.contains("close-modal")||e.target.classList.contains("modal"))&&T()});function A(e,t){const s={features:`<h1>Features Page</h1><p>Features of camper ID: ${t}</p>`,reviews:`<h1>Reviews Page</h1><p>Reviews for camper ID: ${t}</p>`},c=document.getElementById("outlet");c&&(c.innerHTML=s[e]||"<h1>Catalog</h1>"),document.querySelectorAll(".nav-link").forEach(i=>i.classList.remove("active"));const a=document.querySelector(`.nav-link[data-page="${e}"]`);a&&a.classList.add("active")}function O(){document.querySelectorAll(".nav-link").forEach(e=>{e.addEventListener("click",function(t){t.preventDefault();const s=t.target.dataset.id;A(t.target.dataset.page,s)})})}document.addEventListener("click",function(e){if(e.target.closest(".btn-like")){const t=e.target.closest(".btn-like"),s=t.dataset.id;t.classList.toggle("liked");const c=t.querySelector("img"),a=t.classList.contains("liked");c.src=a?"./src/img/red-heart.svg":"./src/img/heart.svg";const i=JSON.parse(localStorage.getItem("likedCampers"))||{};i[s]=a,localStorage.setItem("likedCampers",JSON.stringify(i))}});const P=document.querySelector(".result"),F=document.querySelector(".loader"),p=document.querySelector(".load-more"),R=document.querySelector("#filtersForm");F.style.display="none";p.style.display="none";let n=1;const l=4;let u=[],o=[];async function N(){try{u=await C(),o=u,$(o,l),n=1,w(n,l,o)}catch{P.innerHTML="<p>Not found</p>",p.classList.add("no-active")}}function B(){n++;const e=(n-1)*l,t=n*l;M(o,e,t),w(n,l,o)}R.addEventListener("submit",function(e){e.preventDefault(),o=b(u),$(o,l)});document.querySelectorAll(".container-filters").forEach(e=>{e.addEventListener("click",function(){e.classList.toggle("active-filter")})});document.addEventListener("click",function(e){const t=e.target.closest(".campers-link");if(!t)return;const s=t.dataset.id;window.history.pushState({},"",`/catalog/${s}`),m(s)});document.addEventListener("DOMContentLoaded",()=>{const e=window.location.pathname.split("/").filter(Boolean),s=e[0]==="catalog"&&e[1]?e[1]:null,c=sessionStorage.getItem("openModal");s?m(s):c&&m(c)});p.addEventListener("click",B);window.onload=N;
//# sourceMappingURL=main-C6J7KFC1.js.map
